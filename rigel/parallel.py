...
def parallel(Lx1 ,Ly1, Lx2, Ly2, x0,y0,z):
	x1L = [x0-Lx1/2, x0+Lx1/2]
	x2L = [-Lx2/2, +Lx2/2]
	y1L = [y0-Ly1/2, y0+Ly1/2]
	y2L = [-Ly2/2, +Ly2/2]
	Limits = [x1L, x2L, y1L, y2L]
	return ISum(Limits, z) * k0 / ( (Lx1*Ly1) * (Lx2*Ly2)) *2 /np.sqrt(np.pi)
...
def parallel_coplanar(Lx1 ,Ly1, Lx2, Ly2, x0,y0):
	x1L = [x0-Lx1/2, x0+Lx1/2]
	x2L = [-Lx2/2, +Lx2/2]
	y1L = [y0-Ly1/2, y0+Ly1/2]
	y2L = [-Ly2/2, +Ly2/2]
	Limits = [x1L, x2L, y1L, y2L]
	return ISum_coplanar(Limits) * k0 / ( (Lx1*Ly1)*(Lx2*Ly2)) *2 /np.sqrt(np.pi)
def saeed_sc(x ,y):
...
if __name__ == "__main__":
	d = 0; 	Lx = 1; Ly = 1; z = 1
	Isc = saeed_sc(Lx, Ly)
	print ('saeed self coupling %e' %Isc)
